<template>
  <el-table
    ...
    <!-- column -->
    <template v-else>
      <template v-for="(item, index) in props?.modelValue?.columns">
        <el-table-column
...
function setRowPointer() {
  if (!props.showSelection && !props.dbEdit) {
    return
  }
  // 不加延迟nodes有可能拿不到
  setTimeout(() => {
...
function selectHandle(type: 'checked' | 'remove', index: number, row: any) {
  ...
  if (type === 'checked') {
    ...
    emits('select-add', row)
  } else {
    selectionList.splice(index, 1)
    emits('select-remove', row)
  }
  emits('select', { selection: selectionList, row })
